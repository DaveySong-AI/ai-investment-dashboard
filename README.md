...
AI Briefing 另有唯一的 canonical 格式：`data/ai-briefings/YYYY-MM-DD.json` 是某日期的双语原始记录，`reports/ai-briefings/YYYY-MM-DD.md` 是对应的可审计文本；`latest.json` 仅是指向最新日期的指针，避免产生第二份内容副本。编辑规范见 [`docs/AI_BRIEFING_EDITORIAL.md`](docs/AI_BRIEFING_EDITORIAL.md)，未来 Publisher 的执行约定见 [`docs/AI_BRIEFING_PUBLISHER.md`](docs/AI_BRIEFING_PUBLISHER.md)。
